...
<script runat="server">
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            ...
            If _SIMNDC = "091" Then
                sINCreateAccount(Right(_SIMNDC.Trim, 2) & _SIMMSISDN)
            End If
            If _SIMNDC = "090" Then
                If Left(_SIMMSISDN, 1) > 5 Then
                    sINCreateAccount(Right(_SIMNDC.Trim, 2) & _SIMMSISDN)
                End If
            End If
    ...
    Private Sub sINCreateAccount(strMSISDN As String)
	
	    if blnIsComplimentory(Right(strMSISDN, 9))
			Dim strHTTP As String = "http://10.100.11.10/SOP_IN_CreateAccount.aspx?MSISDN=243" & Right(strMSISDN, 9) & "&Amount=0&OperatorId=Registration&cug=Africell comp SIM"
		else
		        Dim strHTTP As String = "http://10.100.11.10/SOP_IN_CreateAccount.aspx?MSISDN=243" & Right(strMSISDN, 9) & "&Amount=0&OperatorId=Registration&cug=Africell Launch Sep"
		end if

    Dim strResult As String = ""
        
        Dim XMLReader = New XmlTextReader(strHTTP)
        While XMLReader.Read()
            If XMLReader.Name = "MSISDN" Then
                XMLReader.Read()
                strResult = XMLReader.Value.ToString
                XMLReader.Read()
            End If
        End While
    End Sub

    Private function blnIsComplimentory(ByVal strMSISDN As String) As Boolean	
	
        blnIsComplimentory = False
        Try
            Dim lngCC As Long
            Dim dsetGetReplySMS As Data.DataSet
			Dim cnGetReplySMS As New clsMSSQLOledb("sa", "@fricell2014", "10.100.11.10", "afr_sl_registration")
            If cnGetReplySMS.blnOpenConn = True Then
                dsetGetReplySMS = cnGetReplySMS.execute("select count(*) co1 from afr_sl_registration.dbo.complimentary where number = 243'"& strMSISDN &"'")
                lngCC = dsetGetReplySMS("co1")
                If lngCC > 0 Then
                    blnIsComplimentory = True
                End If
